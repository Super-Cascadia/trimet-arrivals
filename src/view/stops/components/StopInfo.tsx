...
export default function StopInfo({ stopLocation } : Props) {
    ...
    return (
        <h2>
            <span className="route-indicators">{getStopRoutes(stopLocation.route)}</span>
            <span className="stop-info">{stopLocation.locid} - {stopLocation.desc} - {stopLocation.dir}</span>
        </h2>
    );
}